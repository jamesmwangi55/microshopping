...
public abstract class BaseMicroserviceVerticle extends AbstractVerticle{
  ...
  public void start() throws Exception {
    ...
    circuitBreaker = CircuitBreaker.create(cbOptions.getString("name", "circuit-breaker"), vertx,
        new CircuitBreakerOptions()
          .setMaxFailures(cbOptions.getInteger("max-failures  "))
          .setTimeout(cbOptions.getLong("timeout", 10000L))
          .setFallbackOnFailure(true)
          .setResetTimeout(cbOptions.getLong("rest-timeout", 30000L))
    );

  }

  //===========================================================
  // provide helper methods to publish various kind of services
  //===========================================================
  ...
  protected Future<Void> publishApiGateway(String host, int port){
    Record record = HttpEndpoint.createRecord("api-gateway", true, host, port, "/",
        null).setType("api-gateway");
    return publish(record);
  }
