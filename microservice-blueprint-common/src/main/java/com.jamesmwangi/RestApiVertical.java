...
public abstract class RestApiVertical extends BaseMicroserviceVerticle {
  ...
  /**
   * Validate if a user exists in the requested scope
   *
   * */
  ...
  // Helper result handler with a request context
  ...
  /**
   * This method generates handler for async methods if REST APIs.
   * Use the result in invoke `toString` as the result. The content type is JSON.
   *
   * */
  ...
  /**
   * This method generates handler for async methods if REST APIs.
   * Use the result directly and user given {@code converter} to convert result to string
   * as the response. The content type is JSON
   *
   * @param context routing context instance
   * @param converter a converter that converts result to string.
   * @param <T> result type
   ...
   * @return  generated handler
   * */
  protected <T> Handler<AsyncResult<T>> resultHandlerNotEmpty(RoutingContext context){
    return ar -> {
      if(ar.succeeded()){
        T res = ar.result();
        if(res == null){
   ...
   * */
    ...
    /**
     * This method generates handler for async methods in REST APIs.
     * The result is not needed only the state of the async result is needed
     *
     * @param context routing context instance
     ...
     * <code>{"message": "delete_success"}</code>
     *
     * @param context routing context instance
     * @return generated handler
     * */
  ...
  protected void notImplemented(RoutingContext context){
    context.response().setStatusCode(501)
        .putHeader("content-head", "application/json")
